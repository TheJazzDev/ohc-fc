...
import { KIND_LABEL, type ArticleBody } from "./types";
    ...
    >
      ...
      >
        <span className="font-heading text-[11px] font-medium tracking-[0.14em] text-fg/70 sm:text-xs">
          {isMatchReport ? article.competition : KIND_LABEL[article.kind]}
        </span>
        {isMatchReport ? (
          ...
          <div className="text-balance font-heading text-3xl leading-[1.05] font-bold sm:text-4xl lg:text-5xl">
            {article.title}
          </div>
        )}
      </div>
      <div className="flex flex-col justify-center gap-3 p-6 sm:gap-4 sm:p-8 lg:p-10">
        ...
        <p className="m-0 text-pretty text-sm leading-relaxed text-muted sm:text-base">{article.excerpt}</p>
