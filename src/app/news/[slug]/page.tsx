...
import { getPublishedArticle } from "@/actions/news";
import { mapDbArticleBody } from "@/components/news/map-db-article";
...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const article = mapDbArticleBody(dbArticle);
  const isMatchReport = article.kind === "MATCH_REPORT" && article.us != null && article.them != null;
    ...
    <>
      ...
      <main className="flex-1">
        {isMatchReport ? (
          ...
          <div
            className="flex aspect-[21/9] max-h-[280px] flex-col justify-end gap-1.5 px-4 py-8 sm:px-8 sm:py-12"
            style={{ background: "linear-gradient(180deg, oklch(0.36 0.1 150), oklch(0.26 0.08 150))" }}
          >
            <div className="mx-auto w-full max-w-3xl">
              <span className="font-heading text-[11px] font-medium tracking-[0.14em] text-fg/70 sm:text-xs">Club news</span>
            </div>
          </div>
        )}

        <article className="mx-auto flex max-w-3xl flex-col gap-5 px-4 py-8 sm:gap-6 sm:px-8 sm:py-12">
          <span className="font-heading text-xs font-medium tracking-[0.12em] text-accent uppercase">
            {isMatchReport ? "Match report" : "Club news"}
          </span>
          ...
          {isMatchReport && (article.startingXi || article.subs) && (
            <div className="flex flex-col gap-2 rounded-md border border-fg/10 bg-surface-2 p-5 shadow-resting [clip-path:polygon(0_0,calc(100%-24px)_0,100%_24px,100%_100%,0_100%)]">
              ...
              {article.startingXi && <p className="m-0 text-sm leading-relaxed text-fg/85">{article.startingXi}</p>}
              {article.subs && <p className="m-0 text-sm leading-relaxed text-muted">{article.subs}</p>}
            </div>
          )}

          <Link
            href={isMatchReport ? "/fixtures" : "/news"}
          ...
          >
            {isMatchReport ? "All results" : "All news"}
          </Link>
        </article>
      </main>
      <SiteFooter />
